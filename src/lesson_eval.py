...
def _has_literary_quote(text: str) -> bool:
    ...
    Ищет:
    - Русские кавычки «...» (U+00AB, U+00BB)
    - Обычные кавычки "..." вокруг текста (цифры/слова)
    - Авторские слова с тире: — сказал, — ответил
    
    Это индикатор того, что в секции есть цитата из произведения,
    а не просто фактологический пересказ.
    """
    import re
    if not text:
        return False
    
    # 1. Русские кавычки «...» — самый надёжный маркер
    if "«" in text and "»" in text:
        return True
    
    # 2. Обычные кавычки "..." вокруг содержательного текста (не цифры/одиночные слова)
    if '"' in text:
        # Ищем паттерн "что-то значимое" где содержимое > 3 символов
        m = re.search(r'"[^"]{4,}"', text)
        if m:
            inner = m.group()[1:-1]
            # Исключаем цены ($"100"), коды, одинокие цифры
            if not re.fullmatch(r'[\d.,$€]+', inner):
                return True
    
    # 3. Тире + прописная буква — авторская речь/цитата
    # Пример: 'Он сказал: — Я помню чудное мгновенье'
    if re.search(r':\s*—\s+[А-ЯA-Z]', text):
        return True
