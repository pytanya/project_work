...
# «автор», «доклад», «реферат», «сообщение» + имя/фамилия. Такие строки — не
# учебный контент, а служебный хром платформы.
_AUTHOR_CHROME_RE = re.compile(
    ...
    # «автор: ...», «автором», «от автора»
    r"|(?:^|\s)(автор[ау]?(?:й\s*)?:?\s|"
    # «доклад », «реферат », «сообщение » + заглавное слово (тема с прописной)
    r"доклад[аеiuя]?[.\s]+[А-ЯA-Z]"
    r"|реферат[аеiuя]?[.\s]+[А-ЯA-Z]"
    r"|сообщен[иья][.\s]+[А-ЯA-Z]"
    # «контрольная работа», «курсовая», «проектная работа» + тема
    r"|контрольн[ааяы][.\s]+[А-ЯA-Z]"
    r"|курсов[аые][.\s]+[А-ЯA-Z]"
    r"|проектн[ааяы][.\s]+[А-ЯA-Z]"
    # «учитель/преподаватель» + ФИО или заглавная буква
    r"(?:учитель[ау]?(?:ем)?|преподавател[аия])[:\s,]*[А-ЯA-Z]"
    # «выполнил(а)» + ФИО ученика (заглавная фамилия)
    r"|выполнил[ааю]\s+[А-ЯA-Z]"
    # «проверил(а)» + ФИО учителя
    r"|проверил[ааю]\s+[А-ЯA-Z]"
    r")",
    re.IGNORECASE,
)
# Местоположения/организации, часто идущие вместе с авторскими метаданными:
# «г. Москва», «МБОУ СОШ №5» и т.п. Усиливают сигнал авторского Chrome.
_AUTHOR_LOCATION_RE = re.compile(
    r"^ г\.[А-ЯA-Z]"           # «г. Название»
    r"| м[Кк]р\.?[А-ЯA-Z]"    # «мкр. Название»
    r"| МБОУ|ГБОУ|СОШ\s*\d"   # названия школ
    r"| лицей\s*\d"
    r"| гимнази[ияю]\s*\d",
    re.IGNORECASE,
)
...
def _is_author_chrome(text: str) -> bool:
    """Строка/фрагмент — метаданные автора публикации, а не учебный контент.

    Проверяет паттерны: «опубликован пользователем», «автор: Имя»,
    «доклад ...», «выполнил ...» и т.п. — служебный хром образовательных порталов.
    Возвращает True если фрагмент похож на сервисный текст сайта.
    """
    return bool(_AUTHOR_CHROME_RE.search(text or "") or _AUTHOR_LOCATION_RE.search(text or ""))
