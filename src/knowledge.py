...
_WEB_NOISE_SUBSTR = ("скидк", "добро пожаловать", "готовые учебные материалы",
                     ...
                     "спецусловия", "подписку", "дипломы", "уроки, тесты, задания",
...
def _is_web_noise(line: str) -> bool:
    ...
    if not s:
        return True
    # Многострочный скрап страницы (целый абзац/блок) может содержать слово
    # «скачать»/«войти» и всё же быть контентом — навигация такой длины не бывает.
    if len(s) > 500:
        return False
