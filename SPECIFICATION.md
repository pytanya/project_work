...
| `subject` / `topic` | **обязательное хотя бы одно** (ядро запроса); если задан `subject` — тема `topic` **обязательна** (поиск материалов идёт по теме, не по предмету); «все/весь учебник» → поиск по предмету | нет — ядро запроса |
