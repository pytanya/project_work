...
describe('KnowledgeGraphPanel', () => {
  ...
  it('показывает темы (без корневого учебника)', () => {
    ...
    expect(screen.getByText('Урок 1: Россия — наша Родина')).toBeInTheDocument()
    expect(screen.getByText('Урок 2: Культура и религия')).toBeInTheDocument()
    expect(screen.queryByText('Учебник «x»')).not.toBeInTheDocument()
  })
  ...
  it('вызывает onSelect по клику', async () => {
    ...
    await user.click(screen.getByText('Урок 2: Культура и религия'))
