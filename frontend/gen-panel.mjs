...
// Build entity strings using char codes
const amp = String.fromCharCode(38) + 'amp;'
const ltS = String.fromCharCode(38) + 'lt;'
const gtS = String.fromCharCode(38) + 'gt;'

const content = `// LessonPanel — урок по теме (режим lesson).
...
export default function LessonPanel({ text, topic, lesson }) {
  /* ── НОВЫЙ ПУТЬ: raw_text из markdown-стриминга ── */
  const rawText = lesson?.raw_text || ''
  if (rawText && typeof rawText === 'string' && rawText.trim().length > 50) {
    // Есть сырой markdown-текст → рендерим как MarkdownLesson
    return <MarkdownLesson rawText={rawText} topic={topic} lesson={lesson} />
  }
  ...
  /* ── Санитизация данных (Hooks вызываем ДО любых условий) ── */
  const data = useMemo(() => {
    if (!raw) return null
    return {
      ...raw,
